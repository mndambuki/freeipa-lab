...
Enroll FreeIPA clients using Ansible in two steps.
...
ansible-playbook ansible/playbooks/provision-hosts.yml \
    ...
    --extra-vars "@ansible/variables/localhost/main.yml" \
    --extra-vars "@ansible/variables/localhost/host-groups.yml" \
    --extra-vars "ipa_hosts_folder=variables/localhost/hosts" \
    --extra-vars "ipa_host_id=*"
----

Then, get into the host an run the following command.
...
Or enroll FreeIPA clients using Ansible in one step.
